# The random Game, it keeps going until the user has the right number
...
def random_game():
    ...
    while True: #The game runs in a while loop that continues until the user exits by entering -1.
        try:
            ...
            # it prints the random number and guessed number in blue
            else: 
                guessed_number == random_number
                print(f"{Fore.BLUE}Congratulation! You guessed the right number: {guessed_number} and {random_number} {Style.RESET_ALL}")
                break
